...
public abstract class BaseClient<T> {
  ...
  protected <R> List<R> postBatch(String url, List<T> body, Map<String, String> headersMap,
      org.springframework.core.ParameterizedTypeReference<List<R>> responseType) {
    HttpHeaders headers = new HttpHeaders();
    if (headersMap != null) {
      headersMap.forEach(headers::set);
    }
    headers.setContentType(MediaType.APPLICATION_JSON);
    HttpEntity<List<T>> request = new HttpEntity<>(body, headers);
    ResponseEntity<List<R>> response = restTemplate.exchange(url, HttpMethod.POST, request, responseType);
    return response.getStatusCode().is2xxSuccessful() ? response.getBody() : null;
  }
